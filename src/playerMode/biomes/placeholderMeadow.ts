...
export class PlaceholderMeadowBiome {
  ...
  private renderGrassGrid(
    ...
    const startX =
      Math.floor(cameraPos.x - viewportSize.width / 2 / scale) * scale;
    const startY =
      Math.floor(cameraPos.y - viewportSize.height / 2 / scale) * scale;
  ...
  private renderDecorations(
    ...
    const viewRadius =
      Math.max(viewportSize.width, viewportSize.height) / 2 + 100;
      ...
      for (let chunkY = startChunkY; chunkY <= endChunkY; chunkY++) {
        ...
        if (!this.decorationCache.has(chunkKey)) {
          this.decorationCache.set(
            chunkKey,
            this.generateChunkDecorations(chunkX, chunkY, chunkSize)
          );
        }
   ...
   */
  private generateChunkDecorations(
    chunkX: number,
    chunkY: number,
    chunkSize: number
  ): DecorationElement[] {
    ...
    for (let i = 0; i < decorationCount; i++) {
      const type =
        this.config.decorations.types[
          Math.floor(rng() * this.config.decorations.types.length)
        ];
   ...
   */
  private renderChunkDecorations(
    ctx: CanvasRenderingContext2D,
    ...
    decorations.forEach((decoration) => {
      const screenX =
        viewportSize.width / 2 + (decoration.x - cameraPos.x) * 50;
      const screenY =
        viewportSize.height / 2 + (decoration.y - cameraPos.y) * 50;

      // Only render if on screen
      if (
        screenX < -20 ||
        screenX > viewportSize.width + 20 ||
        screenY < -20 ||
        screenY > viewportSize.height + 20
      ) {
        return;
      }
  ...
  private renderParallaxLayer(
    ...
    for (let i = 0; i < 5; i++) {
      const x =
        ...
        ((i * 200 - offsetX) % (viewportSize.width + 100)) -
        50;
      const y =
        viewportSize.height / 2 +
        ((i * 150 - offsetY) % (viewportSize.height + 100)) -
        50;
