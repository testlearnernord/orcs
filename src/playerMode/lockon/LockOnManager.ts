...
export class LockOnManager {
   ...
   */
  acquire(candidates: LockOnTarget[], playerPos: Point2D, playerDir: number): string | undefined {
    ...
    const inRangeTargets = validTargets.filter(target => 
      distance(playerPos, target.pos) <= this.lockRadius
    );
   ...
   */
  cycle(candidates: LockOnTarget[], direction: 1 | -1, playerPos: Point2D): string | undefined {
    if (candidates.length === 0) return undefined;
    ...
    const currentIndex = angledTargets.findIndex(t => t.target.id === this.current);
    if (currentIndex === -1) {
    ...
    const nextIndex = (currentIndex + direction + angledTargets.length) % angledTargets.length;
