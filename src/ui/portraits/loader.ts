...
function isDebugGridEnabled(): boolean {
  if (typeof window === 'undefined') {
    return false;
  }
  try {
    return window.localStorage?.getItem('art.debugGrid') === '1';
  } catch {
    return false;
  }
}

function drawDebugGrid(ctx: SliceContext, size: number, tile: number): void {
  const ratio = tile > 0 ? size / tile : 1;
  const step = 256 * ratio;
  const lineWidth = Math.max(1, Math.round(Math.max(ratio, 1)));
  ...
  const call = (method: 'save' | 'restore') => {
    const fn = (ctx as SliceContext & { [key in typeof method]?: () => void })[
      method
    ];
  ...
  (ctx as SliceContext & { strokeStyle?: string }).strokeStyle =
    'rgba(255, 255, 255, 0.45)';
  (ctx as SliceContext & { lineWidth?: number }).lineWidth = lineWidth;
  ...
  const drawLine = (pos: number, horizontal: boolean) => {
    const clamped = Math.max(0, Math.min(size, pos));
    ctx.beginPath();
    if (horizontal) {
      ctx.moveTo(0, clamped);
      ctx.lineTo(size, clamped);
    } else {
      ctx.moveTo(clamped, 0);
      ctx.lineTo(clamped, size);
    }
    ctx.stroke();
  };
  ...
  if (Number.isFinite(step) && step > 0 && step < size) {
    for (let pos = step; pos < size; pos += step) {
      positions.add(pos);
    }
  }
...
export async function sliceTileToURL(
  ...
  const showDebugGrid = isDebugGridEnabled();
  ...
  if (showDebugGrid) {
    drawDebugGrid(ctx as SliceContext, size, tile);
  }
