...
import { HighlightSystem, CinematicHighlightPortal, HighlightType, type EnhancedHighlight } from '@ui/highlights';
...
export class NemesisUI {
  ...
  constructor(
    ...
    this.cinematicPortal = new CinematicHighlightPortal({
      ...
      onToggleEnabled: (enabled) => this.highlightSystem.updateOptions({ enabled }),
    ...
    store.events.on('cycle:completed', (summary) => {
      ...
      // NEW: Generate highlights using the new modular system
      const currentState = this.store.getState();
      // We need the previous state - let's store it or get it from the summary
      // For now, we'll use the summary-based approach since computeDigest already works
      // This will be called by the cycle:digest event below with the computed highlights
    });
    
    // NEW: Process highlights generated by computeDigest using new cinematic system
    store.events.on('cycle:digest', ({ cycle, highlights }) => {
      // For backward compatibility, we need to convert old Highlight[] to EnhancedHighlight[]
      // But since the new system generates from scratch, we'll intercept earlier
      // This is a transitional approach - the store should use the new system eventually
      if (highlights.length > 0) {
        // Convert legacy highlights to enhanced format for display
        const enhancedHighlights = this.convertLegacyHighlights(highlights, cycle);
  ...
  private convertLegacyHighlights(
    ...
    return legacyHighlights.map((legacy) => {
      ...
      let priority = 5;
      let animationType: 'confrontation' | 'celebration' | 'tragedy' | 'emergence' = 'emergence';
      ...
      } else if (legacy.id.startsWith('rival:') || legacy.id.startsWith('ally:')) {
        type = HighlightType.DIPLOMACY;
        priority = 3;
        animationType = 'confrontation';
      } else if (legacy.id.startsWith('rank:') || legacy.id.startsWith('promotion:')) {
