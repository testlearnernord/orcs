...
function totalTiles(atlases: LoadedAtlas[]): number {
  return atlases.reduce((sum, atlas) => {
    const tiles = Math.max(0, Math.floor(atlas.spec.cols) * Math.floor(atlas.spec.rows));
    return sum + tiles;
  }, 0);
}

function selectAtlas(atlases: LoadedAtlas[], globalIndex: number): Selection | null {
...
export const OfficerAvatar: React.FC<OfficerAvatarProps> = ({
  ...
  React.useEffect(() => {
    if (requireTag && import.meta.env.DEV) {
      console.warn('[portraits] requireTag is not supported with atlas slicing');
    }
  }, [requireTag]);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const url = await sliceTileToURL(picked.atlas, picked.index, targetSize);
  ...
  if (!portraitUrl) {
    return (
      <div
        ...
        title={title}
        data-state="loading"
        className={getClassName('officer-avatar officer-avatar--loading', className)}
        style={combinedStyle}
      />
    );
  }
