...
describe('Berserker Sprite Direction Mapping', () => {
  ...
  it('should map directions to correct sprite atlas rows according to Universal LPC standard', () => {
    ...
    // Based on user feedback with LPC generator reference:
    // Row 0: UP (character facing up/north)
    // Row 1: DOWN (character facing down/south)
    // Row 2: RIGHT (character facing right/east)
    // Row 3: LEFT (character facing left/west)
    const { rowByDir } = BERS;

    expect(rowByDir.U).toBe(0); // Up → Row 0
    expect(rowByDir.D).toBe(1); // Down → Row 1  
    expect(rowByDir.R).toBe(2); // Right → Row 2
    expect(rowByDir.L).toBe(3); // Left → Row 3
  });

  it('should generate correct frame indices for walking with Universal LPC mapping', () => {
    // Test walk frame calculation with corrected mapping
