...
describe('Berserker Sprite Direction Mapping', () => {
  ...
  it('should map directions to correct sprite atlas rows', () => {
    ...
    // Universal LPC Spritesheet Generator standard:
    // Row 0: RIGHT, Row 1: DOWN, Row 2: UP, Row 3: LEFT
    const { rowByDir } = BERS;

    expect(rowByDir.D).toBe(1); // Down → Row 1
    expect(rowByDir.L).toBe(3); // Left → Row 3
    expect(rowByDir.R).toBe(0); // Right → Row 0
    expect(rowByDir.U).toBe(2); // Up → Row 2
  });

  it('should generate correct frame indices for walking', () => {
    // Test walk frame calculation with corrected row mapping
    // Universal LPC standard: Row 0=RIGHT, Row 1=DOWN, Row 2=UP, Row 3=LEFT
    expect(idxWalk('D', 'idle')).toBe(9); // Down idle → Row 1, Col 0 (1*9 + 0)
    expect(idxWalk('L', 'idle')).toBe(27); // Left idle → Row 3, Col 0 (3*9 + 0)
    expect(idxWalk('R', 'idle')).toBe(0); // Right idle → Row 0, Col 0 (0*9 + 0)
    expect(idxWalk('U', 'idle')).toBe(18); // Up idle → Row 2, Col 0 (2*9 + 0)

    // Test walk frame 1
    expect(idxWalk('D', 1)).toBe(10); // Down walk frame 1 → Row 1, Col 1
    expect(idxWalk('L', 1)).toBe(28); // Left walk frame 1 → Row 3, Col 1
    expect(idxWalk('R', 1)).toBe(1); // Right walk frame 1 → Row 0, Col 1
    expect(idxWalk('U', 1)).toBe(19); // Up walk frame 1 → Row 2, Col 1
  });
